...
foreach($data_tabla as $tabla){
	...
	fputs($ar,"\n");
	foreach($data_campo as $row){
		$cam_def = '		$this->'.$row["Field"];
		fputs($ar,$cam_def);
		if((substr($row["Type"],0,3)=="int") or (substr($row["Type"],0,3)=="real")){
			$cam_def = '= 0;';
		}else{
			$cam_def = '= "";';
		}
	...
	fputs($ar,"\n");
	foreach($data_campo as $row){
		$cam_def = '	public function  get'.ucfirst($row["Field"]).'() {';
		fputs($ar,$cam_def);
		fputs($ar,"\n");
		$cam_def = '			return $this->'.$row["Field"].';';
		fputs($ar,$cam_def);
		fputs($ar,"\n");
		fputs($ar,"	}");
		fputs($ar,"\n");
		
		$cam_def = ' public function  set'.ucfirst($row["Field"]).'('.'$'.$row["Field"].') {';
		fputs($ar,$cam_def);
		fputs($ar,"\n");
		$cam_def = '       		$this->'.$row["Field"].' = '.'$'.$row["Field"].'; ';
		fputs($ar,$cam_def);
		fputs($ar,"\n");
		fputs($ar,"	}");
		fputs($ar,"\n");
